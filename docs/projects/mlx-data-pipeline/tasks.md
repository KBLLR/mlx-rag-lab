...
| MDP-009 | Define shared document schema between ingestion and VDB | Introduce a shared schema (Pydantic model or dataclass) for document chunks so ingestion and VectorDB share a single definition, enabling future `mlx.data` streaming pipelines. | High     | TBD   |                | Serves as the bridge between ingestion and retrieval layers. |
